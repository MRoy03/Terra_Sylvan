...
interface MessageInputProps {
  onSend:       (content: string, type?: 'text' | 'image' | 'video' | 'sticker', mediaURL?: string) => Promise<void>
  onTyping:     () => void
  onStopTyping: () => void
  disabled?:    boolean
}
...
export function MessageInput({ onSend, onTyping, onStopTyping, disabled }: MessageInputProps) {
  ...
  }, [text, sending, onSend, onStopTyping])
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isCloudinaryConfigured()) {
      toast.error('Cloudinary not set up yet. Add credentials to .env.local')
      return
    }
